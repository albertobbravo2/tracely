...
class extends BackofficeComponent
{
     ...
     * de una sola llamada, y si la API responde 403 el desplegable no se pinta.
     */
    ...
    /**
     * Roles del usuario, ya en texto. Vienen del eager load de `users.index`.
     *
    ...
    public function roleLabel(array $user): string
    {
        ...
        return $roles->isEmpty() ? __('Cliente (sin rol)') : $roles->implode(', ');
    }
};
?>
...
<div class="space-y-6">
                        ...
                        <flux:table.row :key="$user['id']">
                            <flux:table.cell>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span class="font-semibold text-gris-900 dark:text-blanco">{{ $user['name'] }}</span>
                                ...
                                @else
                                    <div class="flex flex-wrap gap-1">
                                        @foreach ($user['roles'] as $role)
                                            <flux:badge rounded size="sm" class="!bg-azul-050 !text-azul-800">
                                                {{ $role['name'] }}
                                            </flux:badge>
                                        @endforeach
                                    </div>
    ...
    <flux:modal name="user-form" class="w-full md:w-[32rem]">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">
                    {{ $editing === null ? __('Nuevo usuario') : __('Editar usuario') }}
                </flux:heading>
                <flux:text class="mt-1">
                    {{ __('Una cuenta sin rol es un cliente final: solo ve sus propios pedidos.') }}
                </flux:text>
            </div>
